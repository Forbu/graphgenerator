...
class DatasetErdos(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        nb_nodes = self.n - (idx % (self.n - self.block_size)) + self.block_size # nb nodes in the current graph
        ...
        node_features = torch.zeros(self.n, 1)
        
        # now we create the block indexes (the last two nodes)
        block_index = torch.tensor([nb_nodes - 2, nb_nodes - 1], dtype=torch.long)
